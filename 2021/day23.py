...
class Burrow:
    pods_1 = (
        ((2, 1), 'B'), ((4, 1), 'A'), ((6, 1), 'B'), ((8, 1), 'C'),
        ((2, 2), 'C'), ((4, 2), 'D'), ((6, 2), 'D'), ((8, 2), 'A'),
    )
    pods_2 = (
    ...
    def __init__(self, part):
        ...
        self.__spaces = {pos: Space(pos, None, typ)
                         for pos, typ in self.spaces_1}
        if part == 2:
            self.__spaces = {
                **self.__spaces,
        ...
        for pos, typ in self.pods_1:
            self.__spaces[(pos[0], 4) if part == 2 and pos[1] ==
                          2 else pos].setOccupant(Amphipod(typ))
        if part == 2:
    ...
    def move(self, direction):
        new_position = (self.selected[0] + direction[0],
                        self.selected[1] + direction[1])
...
while running:
    ...
    for position, space in burrow.spaces.items():
        pygame.draw.rect(screen, Color.SPACE_OUTLINE, (space.getPosition()[
                         0] * TILE_SIZE + 9, space.getPosition()[1] * TILE_SIZE + 59, TILE_SIZE-8, TILE_SIZE-8))
        pygame.draw.rect(screen, Color.SPACE_FILL, (space.getPosition()[
                         0] * TILE_SIZE + 10, space.getPosition()[1] * TILE_SIZE + 60, TILE_SIZE-10, TILE_SIZE-10))

        if not space.isOccupied and space.type in ['A', 'B', 'C', 'D']:
            text = font.render(space.type, True, Color.TEXT_DIM)
            screen.blit(text, (space.getPosition()[
                        0] * TILE_SIZE + 20, space.getPosition()[1] * TILE_SIZE+52))
        ...
        if space.isOccupied:
            ...
            screen.blit(occupant_img, (space.getPosition()[
                        0] * TILE_SIZE + 20, space.getPosition()[1] * TILE_SIZE+52))

    help_text = help_font.render(HELP_TEXT, True, Color.TEXT_WHITE)
    screen.blit(help_text, help_text.get_rect(
        center=(SCREEN_WIDTH/2, 0)).move(0, TILE_SIZE*7-20))
    help_text_2 = help_font.render(HELP_TEXT_2, True, Color.TEXT_WHITE)
    screen.blit(help_text_2, help_text_2.get_rect(
        center=(SCREEN_WIDTH/2, 0)).move(0, TILE_SIZE*7+10))
