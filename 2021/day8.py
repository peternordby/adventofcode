...
for line in input.split('\n'):
    ...
    for seq in input.split():
        if len(seq) == 2:
            ...
            for char in seq:
                seven_seg['two_three_five'].add(char)
        elif len(seq) == 6:
            chars = list(seq)
            seven_seg['zero_six_nine'].append(chars)

    for char in two:
        seven_seg['top_right'].add(char)
        seven_seg['bottom_right'].add(char)

    seven_seg['top'] = set(seven).difference(set(two)).pop()

    for char in set(four).difference(set(two)):
        seven_seg['middle'].add(char)
        seven_seg['top_left'].add(char)

    for char in set(eight).difference(set(four)).difference(set(seven)):
        seven_seg['bottom'].add(char)
        seven_seg['bottom_left'].add(char)

    for char in seven_seg['top_right']:
        if char in seven_seg['zero_six_nine'][0] and char in seven_seg['zero_six_nine'][1] and char in seven_seg['zero_six_nine'][2]:
            seven_seg['bottom_right'] = char
        else:
            seven_seg['top_right'] = char

    for char in seven_seg['bottom_left']:
        if char in seven_seg['zero_six_nine'][0] and char in seven_seg['zero_six_nine'][1] and char in seven_seg['zero_six_nine'][2]:
            seven_seg['bottom'] = char
        else:
            seven_seg['bottom_left'] = char

    for char in seven_seg['middle']:
        if char in seven_seg['zero_six_nine'][0] and char in seven_seg['zero_six_nine'][1] and char in seven_seg['zero_six_nine'][2]:
            seven_seg['top_left'] = char
        else:
            seven_seg['middle'] = char

    num = ''
    for seq in output.split():
        if len(seq) == 2:
            num += '1'
        elif len(seq) == 3:
            num += '7'
            ...
            keys = list(seven_seg.keys())
            vals = list(seven_seg.values())
            segments = []
            for char in seq:
                i = vals.index(char)
                segments.append(keys[i])
            if len(seq) == 5:
                # two
                if 'top' in segments and 'top_right' in segments and 'middle' in segments and 'bottom_left' in segments and 'bottom' in segments:
                    num += '2'
                # three
                if 'top' in segments and 'top_right' in segments and 'middle' in segments and 'bottom_right' in segments and 'bottom' in segments:
                    num += '3'
                # five
                if 'top' in segments and 'top_left' in segments and 'middle' in segments and 'bottom_right' in segments and 'bottom' in segments:
                    num += '5'

            elif len(seq) == 6:
                # zero
                if 'top' in segments and 'top_right' in segments and 'top_left' in segments and 'bottom_left' in segments and 'bottom' in segments and 'bottom_right' in segments:
                    num += '0'
                # six
                if 'top' in segments and 'top_left' in segments and 'middle' in segments and 'bottom_left' in segments and 'bottom' in segments and 'bottom_right' in segments:
                    num += '6'
                # nine
                if 'top' in segments and 'top_right' in segments and 'top_left' in segments and 'middle' in segments and 'bottom_right' in segments and 'bottom' in segments:
                    num += '9'
                pass
    total += int(num)
