...
for passport in passports:
    ...
    for section in sections:
        letters[section[0:3]] = section[4:]
    if "byr" in letters and "iyr" in letters and "eyr" in letters and "hgt" in letters and "hcl" in letters and "ecl" in letters and "pid" in letters:
        if 2002 >= int(letters["byr"]) >= 1920:
            if 2020 >= int(letters["iyr"]) >= 2010:
                if 2030 >= int(letters["eyr"]) >= 2020:
                    try:
                        heightcm = [letters["hgt"][0:3], letters["hgt"][3:5]]
                        heightin = [letters["hgt"][0:2], letters["hgt"][2:4]]
                    except:
                        continue
                    if (heightcm[1] == 'cm' and 150 <= int(heightcm[0]) <= 193) or heightin[1] == 'in' and 59 <= int(heightin[0]) <= 76:
                        regex = re.compile('#[a-f0-9]{6}', re.I)
                        if bool(regex.match(letters["hcl"])) and len(letters["hcl"]) == 7:
                            valid_ecl = ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]
                            if letters["ecl"] in valid_ecl:
                                regex = re.compile('[0-9]{9}', re.I)
                                if bool(regex.match(letters["pid"])) and len(letters["pid"]) == 9:
                                    valid += 1
