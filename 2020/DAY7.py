input = """posh crimson bags contain 2 mirrored tan bags, 1 faded red bag, 1 striped gray bag.
...
def find_bags(dict, bags):
    ...
    return outerbags + find_bags(dict, outerbags)
...
for line in lines:
    node, rootlist = line.split(" bags contain ")
    roots = rootlist.split(", ")
    bag_dict[node] = []
    for root in roots:
        if root[0] != "n":
            bag_dict[node].append((root.split(" bag")[0][0], root.split(" bag")[0][2:]))
        else:
            bag_dict[node] = []

def search(bag):
    num = 1
    for inner in bag_dict[bag]:
        multiplier = int(inner[0])
        num += multiplier * search(inner[1])
    return num

innerbags = search("shiny gold")
print("the answer is:", innerbags-1)
